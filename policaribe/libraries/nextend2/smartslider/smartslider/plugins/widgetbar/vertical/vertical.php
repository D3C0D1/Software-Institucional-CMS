...
class N2SSPluginWidgetBarVertical extends N2SSPluginWidgetAbstract {
    ...
    public function render($slider, $id, $params) {
        ...
        for ($i = 0; $i < count($slider->slides); $i++) {
            ...
            $title = $slider->slides[$i]->getTitle();
            if (!empty($title)) {
                $title = N2Translation::_($title);
                if (!empty($title)) {
                    $slides[$i]['html'] .= N2Html::tag('div', array('class' => $fontTitle . ' n2-ow'), $title);
                }
            }
        ...
        $slider->features->addInitCallback('new N2Classes.SmartSliderWidgetBarVertical(this, ' . json_encode($slides) . ', ' . json_encode($parameters) . ');');
