...
class SppagebuilderAddonArticles extends SppagebuilderAddons {
    ...
    public function render() {
        ...
        $show_hits = (isset($this->addon->settings->show_hits)) ? $this->addon->settings->show_hits : 1;
        ...
        if (count($items)) {
            ...
            foreach ($items as $key => $item) {
                ...
                $output .= '<h3 class="title"><a href="' . $item->link . '" itemprop="url">' . $item->title . '</a></h3>';
                if ($show_date) {
                    $output .= '<div class="sppb-meta-date" itemprop="dateCreated"> <i class="fa fa-clock-o"></i>' . Jhtml::_('date', $item->created, 'DATE_FORMAT_LC3') . '</div>';
                }
                if ($show_author || $show_category) {
                    $output .= '<div class="sppb-article-meta">';

                    if ($show_category) {
                        $output .= '<span class="sppb-meta-category"><i class="fa fa-tags"></i> <a href="' . JRoute::_(ContentHelperRoute::getCategoryRoute($item->catslug)) . '" itemprop="genre">' . $item->category . '</a></span>';
                    }
