...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $contenido_completo = $_POST['contenido_completo']; // Sin sanitize para mantener HTML
    $autor = sanitize($_POST['autor']);
    $fecha_publicacion = sanitize($_POST['fecha_publicacion']);
    $estado = sanitize($_POST['estado']);
    $estado = in_array($estado, ['borrador','publicado','archivado']) ? $estado : 'publicado';
    ...
    if (isset($_FILES['imagen_principal']) && $_FILES['imagen_principal']['error'] === 0) {
        ...
        if (is_array($resultado) && !empty($resultado['exito'])) { $imagen_principal = $resultado['ruta']; }
        elseif (is_string($resultado) && !empty($resultado)) { $imagen_principal = $resultado; }
        else { $errores[] = is_array($resultado) && isset($resultado['error']) ? $resultado['error'] : 'No fue posible subir la imagen principal'; }
    } else {
        $errores[] = "La imagen principal es obligatoria";
    }

    // Procesar galería de imágenes adicionales
    $imagenes_galeria = [];
    if (isset($_FILES['galeria_imagenes']) && is_array($_FILES['galeria_imagenes']['name'])) {
        $count = count($_FILES['galeria_imagenes']['name']);
        for ($i = 0; $i < $count; $i++) {
            if ($_FILES['galeria_imagenes']['error'][$i] === 0 && !empty($_FILES['galeria_imagenes']['name'][$i])) {
                $archivo = [
                    'name' => $_FILES['galeria_imagenes']['name'][$i],
                    'type' => $_FILES['galeria_imagenes']['type'][$i],
                    'tmp_name' => $_FILES['galeria_imagenes']['tmp_name'][$i],
                    'error' => $_FILES['galeria_imagenes']['error'][$i],
                    'size' => $_FILES['galeria_imagenes']['size'][$i]
                ];
                $res = subirImagen($archivo);
                if (is_array($res) && !empty($res['exito'])) { $imagenes_galeria[] = $res['ruta']; }
                elseif (is_string($res) && !empty($res)) { $imagenes_galeria[] = $res; }
            }
        }
        if (!empty($imagenes_galeria)) {
            foreach ($imagenes_galeria as $url) {
                $contenido_completo .= '<p><img src="' . htmlspecialchars($url) . '" alt=""></p>';
            }
        }
    }
    ...
    if ($destacado && $estado === 'publicado') {
        try {
            $stmt = $db->query("SELECT COUNT(*) FROM articulos WHERE destacado = 1 AND estado = 'publicado'");
    ...
    if (empty($errores)) {
        try {
            ...
            $stmt = $db->prepare("SELECT COUNT(*) FROM articulos WHERE slug = :slug");
            ...
            $sql = "INSERT INTO articulos 
                    (titulo, slug, descripcion_corta, contenido_completo, imagen_principal, 
                     autor, fecha_publicacion, estado, destacado, fecha_creacion) 
                    VALUES 
                    (:titulo, :slug, :descripcion_corta, :contenido_completo, :imagen_principal, 
                     :autor, :fecha_publicacion, :estado, :destacado, NOW())";
            ...
            setMensaje('Artículo creado exitosamente', 'success');
...
<head>
    ...
    <title>Crear Artículo - Panel de Administración</title>
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" 
                                                  ...
                                                  rows="3" 
                                                  required><?php echo isset($descripcion_corta) ? htmlspecialchars($descripcion_corta) : ''; ?></textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea id="contenido_completo" 
                                                  name="contenido_completo"><?php echo isset($contenido_completo) ? $contenido_completo : ''; ?></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" 
                                               ...
                                               id="autor" 
                                               name="autor" 
                                               value="<?php echo isset($autor) ? htmlspecialchars($autor) : htmlspecialchars($_SESSION['usuario_nombre']); ?>"
                                               required>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="datetime-local" 
                                               ...
                                               value="<?php echo isset($fecha_publicacion) ? $fecha_publicacion : date('Y-m-d\TH:i'); ?>"
                                               required>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="estado" name="estado" required>
                                            <option value="borrador" <?php echo (isset($estado) && $estado === 'borrador') ? 'selected' : ''; ?>>Borrador</option>
                                            <option value="publicado" <?php echo (!isset($estado) || $estado === 'publicado') ? 'selected' : ''; ?>>Publicado</option>
                                            <option value="archivado" <?php echo (isset($estado) && $estado === 'archivado') ? 'selected' : ''; ?>>Archivado</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="file" 
                                               ...
                                               accept="image/*"
                                               required>
                                        <small class="text-muted">Formatos aceptados: JPG, PNG, GIF, WEBP, SVG, AVIF (máx. 8MB)</small>
                                    </div>

                                    <div class="form-group">
                                        <label for="galeria_imagenes">Galería (múltiples imágenes)</label>
                                        <input type="file" 
                                               class="form-control" 
                                               id="galeria_imagenes" 
                                               ...
                                               accept="image/*"
                                               multiple>
                                        <small class="text-muted">Sube varias imágenes para que aparezcan en el carrusel del artículo.</small>
                                    </div>

                                    <div class="form-group">
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="destacado" value="1" <?php echo (isset($destacado) && $destacado) ? 'checked' : ''; ?>>
                                                ...
                                                <div class="checkbox">
                                                    <label>
                                                        <input type="checkbox" 
                                                               ...
                                                               <?php echo (isset($categorias_seleccionadas) && in_array($cat['id'], $categorias_seleccionadas)) ? 'checked' : ''; ?>>
